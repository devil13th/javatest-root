package com.thd.activemq.ptp.duration;
...
public class Producer {
	...
	public void start(){
        try {
            ...
            destination = session.createQueue("ptp-message-non persistent");
            //从session中，获取一个消息生产者
        	 ...
        	 * 
        	 * 如果设置为非持久化,则队列中未被消费的消息在MQ宕机再次重新启动后 [会丢失]
        	 * 如果设置为持久化,则队列中未被消费的消息在MQ宕机再次重新启动后 [不会丢失]
        	 * 如果不设置持久化模式 ,发送到队列中的消息 [默认是持久化的]
        	 */
