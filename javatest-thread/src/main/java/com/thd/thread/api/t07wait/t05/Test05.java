package com.thd.thread.api.t07wait.t05;
...
Exception in thread " subtractThread2 " java.lang.IndexOutOfBoundsException: Index 0 out-of-bounds for length 0
	...
	at com.thd.thread.api.t07wait.t04.ThreadSubtract.run(Test04.java:84)
		*
		* 因为subtractThread1和subtractThread2两个线程同时在等待，notifyAll后两个都被唤醒，第一个走下去后 ValueObject.list已经被remove(0)了，
		* 所以当subtractThread2再次remove(0)后会被抛出异常,
		* 所以在obj.wait()外层做判断时，要用while，不要用if， while是循环，当wait()后被唤醒时可以再次验证条件是否满足(防止判断是否执行obj.wait()的时候条件发生了改变)，如t06包下的Test06
		*/
